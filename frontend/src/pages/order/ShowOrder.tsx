...
const ShowOrders = () => {
  ...
  {orders.length === 0 ? (
    ...
    <ul className="space-y-6">
      {orders.map((order) => (
        <li key={order.order_id} className="bg-white shadow-md rounded-lg p-4">
          <div className="flex justify-between items-center">
            <h3 className="text-xl font-semibold text-gray-700">Order ID: {order.order_id}</h3>
            <p className="text-lg font-semibold text-gray-900">Total: ${order.total_price}</p>
          </div>
          ...
          <div className="mt-4">
            <h4 className="text-lg font-medium text-gray-800">Products:</h4>
              <ul className='space-y-2 mt-2'>
                <li className="text-gray-700 font-medium">Product: {order.product_name}</li>
                <li className="text-sm text-gray-600">Quantity: {order.quantity}</li>
                <li className="text-sm text-gray-600">Price: ${order.unit_price}</li>
              </ul>
          </div>
        </li>
      ))}
